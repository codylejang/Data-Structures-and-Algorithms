...
int Cyborg::col() const // returns column of cyborg
{
    return m_col;
}

int Cyborg::channel() const // return which channel cyborg is listening on
{
    return m_channel;
}

bool Cyborg::isDead() const // return whether cyborg is dead
{
    if (m_health <=0){
        return true;
    }
...
void Cyborg::forceMove(int dir) // moves cyborg in direction and inputs damage if fail
{
...
int Player::row() const // returns row player is at
{
    return m_row;
}

int Player::col() const // returns column player is at
{
    return m_col;
}
...
string Player::move(int dir)
{
    ...
    if (m_arena->numberOfCyborgsAt(m_row, m_col)>0) { // if player lands on cyborg
        setDead();
    }
...
bool Player::isDead() const // check if player is dead
{
    if (m_dead == true){
        return true;
    }
...
int Arena::rows() const // return num rows
{
    return m_rows;
}

int Arena::cols() const // return num columns
{
    return m_cols;
}
...
int Arena::cyborgCount() const // return num cyborgs
{
    return m_nCyborgs;
}
...
int Arena::numberOfCyborgsAt(int r, int c) const // returns the number of cyborgs at r , c
{
    int count = 0;
    for (int i = 0; i<m_nCyborgs; i++){
...
void Arena::display(string msg) const
{
    ...
    for (int i = 0; i < m_nCyborgs; i++){
        int row = m_cyborgs[i]->row() - 1;
        int col = m_cyborgs[i]->col() - 1;
        displayGrid[row][col]=m_cyborgs[i]->channel()+48;
    }
...
string Arena::moveCyborgs(int channel, int dir)
{
    ...
    for (int i=0; i<m_nCyborgs; i++){
        if (m_cyborgs[i]->channel()==channel && willRespond==true){ //cyborgs on channel will move specific direction if willRespond is true
            m_cyborgs[i]->forceMove(dir);
        }
        
        if (m_cyborgs[i]->channel()!=channel && willRespond==true){ //willRespond is true but cyborg is not on channel and moves randomly
            m_cyborgs[i]->move();
        }
        
        if (willRespond==false){ //if willRespond is false cyborg moves randomly
            m_cyborgs[i]->move();
        }
        
        if (m_cyborgs[i]->row() == m_player->row() && m_cyborgs[i]->col() == m_player->col()){ // if cyborg is on same space as player, player is dead
            m_player->setDead();
        }
        
        if (m_cyborgs[i]->isDead()==true){
            delete m_cyborgs[i];
...
int countSurround (const Arena& a, int r, int c){
    int total = a.numberOfCyborgsAt(r - 1, c) + a.numberOfCyborgsAt(r + 1, c) + a.numberOfCyborgsAt(r, c - 1) + a.numberOfCyborgsAt(r, c + 1); // counts all surrounding cyborgs at hypothetical location
    return total;
}
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    const int resetr = r; //resets variables after attemptMove is called
    const int resetc = c;
    int safe = countSurround (a, r, c);
    int idealpos=99;
    
    if (attemptMove(a, 0, r, c) && a.numberOfCyborgsAt(r, c)==0){  //case for north
        int newsafe = countSurround(a, r, c);
        if (newsafe<safe){
            safe=newsafe;
            idealpos=0;
        }
    ...
    r=resetr; //resets variables after attemptMove is called
    c=resetc;
    
    if (attemptMove(a, 1, r, c) && a.numberOfCyborgsAt(r, c)==0){  //case for east
        int newsafe = countSurround(a, r, c);
        if (newsafe<safe){
            safe=newsafe;
            idealpos=1;
    ...
    if (attemptMove(a, 2, r, c) && a.numberOfCyborgsAt(r, c)==0){  //case for south
        int newsafe = countSurround(a, r, c);
        if (newsafe<safe){
            safe=newsafe;
            idealpos=2;
    ...
    if (attemptMove(a, 3, r, c) && a.numberOfCyborgsAt(r, c)==0){  //case for west
        int newsafe = countSurround(a, r, c);
        if (newsafe<safe){
            safe=newsafe;
            idealpos=3;
